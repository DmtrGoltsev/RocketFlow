...
class SharingIntegrationTest {
    ...
    void goalShareAcceptEnablesCollaborativeGoalAndTaskAccess() throws Exception {
        ...
        String ownerTaskAfterCollaboratorUpdate = mockMvc.perform(get("/api/tasks/" + taskId)
                ...
                .andExpect(jsonPath("$.title").value("Prepare promotion plan"))
                .andReturn()
                .getResponse()
                .getContentAsString();
    ...
    void directTaskShareDeclineDuplicateAndRevokeAreEnforced() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks/" + taskId)
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("todo"))
                .andExpect(jsonPath("$.title").value("Prepare status update"));
    ...
    void shareScopesFilterUnsharedObjectsAndSharedTaskStatusUpdatePersists() throws Exception {
        ...
        mockMvc.perform(get("/api/shares/resources")
                ...
                .andExpect(jsonPath("$.folders[0].canAccessFolderContent").value(true))
                .andExpect(jsonPath("$.goals.length()").value(1))
    ...
    void folderShareByUserIdAndShareLinksExposeReadOnlyResources() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks/" + taskId)
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("todo"))
                .andExpect(jsonPath("$.title").value("Write backend contract"));
