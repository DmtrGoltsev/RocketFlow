...
class AuthRepositoryRefreshRaceInstrumentedTest {
    ...
    fun concurrentRefreshAcrossRepositoryInstancesIsSingleFlight() = runBlocking {
        ...
        sessionStore.writeSession(initial)
        ...
        assertEquals("refresh-new", sessionStore.readSession()?.tokens?.refreshToken)
    ...
    fun newerStoredSessionDuringDelayedSuccessfulRefreshIsNotOverwritten() = runBlocking {
            ...
            when {
                ...
                request.method == "GET" && request.path == "/api/me" ->
                    if (request.accessToken == "access-rotated-old") {
                        currentUserResponse("access-new")
                    } else {
                        TestResponse.unauthorized()
                    }
                request.method == "POST" && request.path == "/api/auth/refresh" -> {
        ...
        assertEquals("refresh-newer", sessionStore.readSession()?.tokens?.refreshToken)
    }
    ...
    private fun repository(): AuthRepository {
        ...
        return AuthRepository(HttpJsonClient(baseUrl), SessionStore(context))
    }
